...
tags {"check-rdb external:skip logreqres:skip"} {
    test {Check old valid RDB} {
            ...
            exec $::NEXCACHE_CHECK_RDB_BIN tests/assets/encodings.rdb
        } result
    ...
    test {Check foreign RDB without unknown data} {
            ...
            exec $::NEXCACHE_CHECK_RDB_BIN tests/assets/encodings-rdb12.rdb
        } result
    ...
    test {Check foreign RDB with unknown data} {
            ...
            exec $::NEXCACHE_CHECK_RDB_BIN tests/assets/encodings-rdb75-unknown-types.rdb
        } result
...
tags {"check-rdb network external:skip logreqres:skip"} {
    start_server {} {
        test "test nexcache-check-rdb stats with empty RDB" {
            r flushall
            r save
            set dump_rdb [file join [lindex [r config get dir] 1] dump.rdb]
            catch {
                exec $::NEXCACHE_CHECK_RDB_BIN $dump_rdb --stats --format info
            } result
        ...
        test "test nexcache-check-rdb stats function" {
            set function_num 11
            ...
            set dump_rdb [file join [lindex [r config get dir] 1] dump.rdb]
                ...
                exec $::NEXCACHE_CHECK_RDB_BIN $dump_rdb
            } result
            assert_match "*$function_num functions*" $result
        }

        test "test nexcache-check-rdb stats key space" {
            r select 0
