        ...
        test "eviction due to output buffers of many MGET clients, client eviction: $client_eviction" {
            ...
            while {![check_eviction_test $client_eviction] && [expr [clock seconds] - $t] < 20} {
                ...
                if {$client_eviction == "false" && $::verbose} {
                    puts "T=[expr [clock seconds] - $t] used_memory: [format "%.2f MB" [expr [s used_memory] / 1048576.0]], mem_limit: [format "%.2f MB" [expr [lindex [r config get maxmemory] 1] / 1048576.0]], dbsize: [r dbsize], evicted_keys: [s evicted_keys]"
                }
                foreach rr $clients {
                    if {[catch {
                        $rr mget 1
